...
TriFiltWidthSAC=5;
TriFiltWidthDC=1;
TriFiltWidthSC=1;   % CHANGED 9/27/08
...
if ~isfield(paramsOUT, 'SCCdur_sec')
...
paramsOUT.BOOTSTRAP_percentdata=.5;  % Percent of AN reps to include in each RUN (Not low to have highly variable estimates, not high to have increased overlap during bootstrapping)
...
paramsOUT.BOOTSTRAP_Navgs=BootstrapLoopMax;  % Number of RUNS for Averaging CCCs, or PSDs
BOOTinds=cell(3,2);
for i=1:3
    for j=1:2
...
for BOOTrep1=1:BootstrapLoopMax
    for i=1:3
        for j=1:2
            BOOTinds{i,j}{BOOTrep1}=randsample(Nreps(i,j),Ninclude(i,j));
        end
    end
end
...
SACSCCmetrics=cell(BootstrapLoopMax,1);
SACSCCfunctions=cell(BootstrapLoopMax,1);


%% Variables/Codes for parfor to work
...
%% Main Loop
for BOOTrep=1:BootstrapLoopMax
    % parfor BOOTrep=1:BootstrapLoopMax
    % Run BOOTSTRAP_Navgs different random samples
    
    %% Window spike times (ignore 1st 50 ms, and cut to shorter of 2 stim)
    [ST_A_plus,NumDrivenSpikes11,nLines11]=Library.windowSTs(SpikeTrains{1,1}(BOOTinds{1,1}{BOOTrep}), ...
        paramsOUT.SCC_onsetIGNORE_sec,stimdur_sec,paramsOUT.MAXspikes); %#ok<*PFBNS>
    [ST_A_minus,NumDrivenSpikes12,nLines12]=Library.windowSTs(SpikeTrains{1,2}(BOOTinds{1,2}{BOOTrep}), ...
        paramsOUT.SCC_onsetIGNORE_sec,stimdur_sec,paramsOUT.MAXspikes);
    [ST_B_plus,NumDrivenSpikes21,nLines21]=Library.windowSTs(SpikeTrains{2,1}(BOOTinds{2,1}{BOOTrep}), ...
        paramsOUT.SCC_onsetIGNORE_sec,stimdur_sec,paramsOUT.MAXspikes);
    [ST_B_minus,NumDrivenSpikes22,nLines22]=Library.windowSTs(SpikeTrains{2,2}(BOOTinds{2,2}{BOOTrep}), ...
        paramsOUT.SCC_onsetIGNORE_sec,stimdur_sec,paramsOUT.MAXspikes);
    [ST_C_plus,NumDrivenSpikes31,nLines31]=Library.windowSTs(SpikeTrains{3,1}(BOOTinds{3,1}{BOOTrep}), ...
        paramsOUT.SCC_onsetIGNORE_sec,stimdur_sec,paramsOUT.MAXspikes);
    [ST_C_minus,NumDrivenSpikes32,nLines32]=Library.windowSTs(SpikeTrains{3,2}(BOOTinds{3,2}{BOOTrep}), ...
        paramsOUT.SCC_onsetIGNORE_sec,stimdur_sec,paramsOUT.MAXspikes);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     fprintf('NUMBER OF SPIKES (A_plus, A_minus, B_plus, B_minus, C_plus, C_minus) = %d, %d, %d, %d, %d, %d\n',...
%         NumDrivenSpikes11,NumDrivenSpikes12,NumDrivenSpikes21,NumDrivenSpikes22,NumDrivenSpikes31,NumDrivenSpikes32);
%     fprintf('NUMBER OF REPS (A_plus, A_minus, B_plus, B_minus, C_plus, C_minus) = %d, %d, %d, %d, %d, %d\n',...
...
%     fprintf('   ... Computing SACs/SCCs for AN spikes: REP %d of %d ...\n',BOOTrep,BootstrapLoopMax);
    ...
    if min([NumDrivenSpikes11,NumDrivenSpikes12,NumDrivenSpikes21,NumDrivenSpikes22,NumDrivenSpikes31,NumDrivenSpikes32])>paramsOUT.MINspikes
        
        [SACSCCs,AvgRate_sps]=SACSCCanal(ST_A_plus,ST_A_minus,ST_B_plus,ST_B_minus,ST_C_plus,ST_C_minus,paramsOUT, winCorr0Add1Mul);
        ...
        STrand_A_plus=Library.randomizeSTs(ST_A_plus,paramsOUT.curBoundaries(1),paramsOUT.curBoundaries(2));
        STrand_A_minus=Library.randomizeSTs(ST_A_minus,paramsOUT.curBoundaries(1),paramsOUT.curBoundaries(2));
        STrand_B_plus=Library.randomizeSTs(ST_B_plus,paramsOUT.curBoundaries(1),paramsOUT.curBoundaries(2));
        STrand_B_minus=Library.randomizeSTs(ST_B_minus,paramsOUT.curBoundaries(1),paramsOUT.curBoundaries(2));
        STrand_C_plus=Library.randomizeSTs(ST_C_plus,paramsOUT.curBoundaries(1),paramsOUT.curBoundaries(2));
        STrand_C_minus=Library.randomizeSTs(ST_C_minus,paramsOUT.curBoundaries(1),paramsOUT.curBoundaries(2));
        
%         fprintf('   ... Computing SACs/SCCs for RANDOM spikes (for Noise Floor) ...\n');
        [SACSCCs_rand,~]=SACSCCanal(STrand_A_plus,STrand_A_minus,STrand_B_plus,STrand_B_minus,STrand_C_plus,STrand_C_minus,paramsOUT, winCorr0Add1Mul);
...
% % % %             [~,PSD_LHfreqs_inds(i,1)]=min(abs(SACSCCs.PSD_freqVEC-paramsOUT.PSD_LHfreqs_Hz(i,1)));
% % % %             [~,PSD_LHfreqs_inds(i,2)]=min(abs(SACSCCs.PSD_freqVEC-paramsOUT.PSD_LHfreqs_Hz(i,2)));
% % % %         end
% % % %         
...
% % % %         SACSCCmetrics{BOOTrep}.sums.PSD_LHfreqsTfs_Hz=paramsOUT.PSD_LHfreqs_Hz;
        ...
        %% NEED TO IMPROVE - find largest, unless there is a "close-to-largest" that %% is closer to zero!
        ...
        %% SAC/DC/SC Peak Heights (SAC peaks - this is CI from Joris et al 2006 (HR) [don't subtract 1])
...
% % % %         SACSCCmetrics{BOOTrep}.SACpeaks_legend{1}='SACpeak_max';
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(2)=1+(max(SACSCCs.SUMCOR_AC)-1)* ...
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(3)=1+(max(SACSCCs.SUMCOR_AC)-1)* ...
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(4)=max(SACSCCs.SUMCORadj_AC);
        % % % %     end
        % % % %     % 5) IFFTraw (0 delay)
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(5)=SACSCCs.SUMCORadj_AC(INDEX_0);
        % % % %     end
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC_5(INDEX_0);
        % % % %     end
        % % % %
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC_64(INDEX_0);
        % % % %     end
        % % % %
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC_300(INDEX_0);
        % % % %     end
        % % % %
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC_5_64(INDEX_0);
        % % % %     end
        % % % %
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC_64_300(INDEX_0);
        % % % %     end
        % % % %     % 6) IFFTraw (CD delay)
        ...
        % % % %         SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC(INDEX_CD);
        % % % %     end
        % % % %     %% User-passed list of delays to compute
        % % % %     if isfield(paramsOUT,'UserDelays_usec')
        % % % %         for i=1:length(paramsOUT.UserDelays_usec)
        ...
        % % % %                 SACSCCmetrics{BOOTrep}.SCpeaks_C(end+1)=SACSCCs.SUMCORadj_AC(INDEX_user);
        % % % %             end
        % % % %         end
        % % % %     end
        ...
        % % % %             else
        % % % %                 SACSCCmetrics{BOOTrep}.CCCenvs_legend{(i-1)*2+1}=...
        % % % %                     sprintf('%.f-%.f, subBIAS',paramsOUT.PSD_LHfreqs_Hz(i,1),paramsOUT.PSD_LHfreqs_Hz(i,2));
        % % % %                 SACSCCmetrics{BOOTrep}.CCCenvs_legend{(i-1)*2+2}=...
        % % % %                     sprintf('%.f-%.f, withBIAS',paramsOUT.PSD_LHfreqs_Hz(i,1),paramsOUT.PSD_LHfreqs_Hz(i,2));
        % % % %             end
        % % % %             % subBIAS
        ...
        % % % %         SACSCCmetrics{BOOTrep}.CCCenvs(end+1) = (SACSCCmetrics{BOOTrep}.SCpeaks_C(RAWSCindex)-1)/ ...
        ...
        % % % %         SACSCCmetrics{BOOTrep}.CCCenvs(end+1) = (SACSCCmetrics{BOOTrep}.SCpeaks_C(ADJSCindex)-1)/ ...
        ...
        % % % %         SACSCCmetrics{BOOTrep}.CCCenvs(end+1) = (SACSCCmetrics{BOOTrep}.SCpeaks_C(IFFTSCindex)-1)/ ...
        ...
        % % % %         SACSCCmetrics{BOOTrep}.CCCenvs(end+1) = (SACSCCmetrics{BOOTrep}.SCpeaks_C(IFFTSC_0_index)-1)/ ...
        ...
        % % % %         SACSCCmetrics{BOOTrep}.CCCenvs(end+1) = (SACSCCmetrics{BOOTrep}.SCpeaks_C(IFFTSC_CD_index)-1)/ ...
        % % % %             (sqrt((SACSCCmetrics{BOOTrep}.SCpeaks_A(IFFTSC_0_index)-1)*(SACSCCmetrics{BOOTrep}.SCpeaks_B(IFFTSC_0_index)-1)));
        % % % %         % User-passed list of delays to compute
        % % % %         if isfield(paramsOUT,'UserDelays_usec')
        % % % %             for i=1:length(paramsOUT.UserDelays_usec)
        ...
        % % % %                 % NOTE: peaks for ACFs are taken at 0 delay (by definition),
        % % % %                 % rather than at specified CCF delay
        % % % %                 SACSCCmetrics{BOOTrep}.CCCenvs(end+1) = (SACSCCmetrics{BOOTrep}.SCpeaks_C(IFFTSC_user_index)-1)/ ...
        % % % %                     (sqrt((SACSCCmetrics{BOOTrep}.SCpeaks_A(IFFTSC_0_index)-1)*(SACSCCmetrics{BOOTrep}.SCpeaks_B(IFFTSC_0_index)-1)));
        % % % %             end
        % % % %         end
        % % % %
        ...
        SACSCCfunctions{BOOTrep}.PSDenv_A = SACSCCs.PSDenv_A;
        SACSCCfunctions{BOOTrep}.PSDenv_B = SACSCCs.PSDenv_B;
        SACSCCfunctions{BOOTrep}.PSDenv_C= SACSCCs.PSDenv_C;
        SACSCCfunctions{BOOTrep}.PSD_freqVEC = SACSCCs.PSD_freqVEC;
        
        SACSCCfunctions{BOOTrep}.delays_usec = SACSCCs.delays_usec;
        SACSCCfunctions{BOOTrep}.SUMCOR_A = SACSCCs.SUMCOR_A;
        SACSCCfunctions{BOOTrep}.SUMCOR_B = SACSCCs.SUMCOR_B;
        SACSCCfunctions{BOOTrep}.SUMCOR_C = SACSCCs.SUMCOR_C;
        ...
        SACSCCmetrics{BOOTrep}.NumDrivenSpikes= [NumDrivenSpikes11,NumDrivenSpikes12;NumDrivenSpikes21,NumDrivenSpikes22;NumDrivenSpikes31,NumDrivenSpikes32];
        SACSCCmetrics{BOOTrep}.nLines=[nLines11,nLines12;nLines21,nLines22;nLines31,nLines32];
    end
end
SACSCCfunctions=SACSCCfunctions(~cellfun('isempty',SACSCCfunctions));
SACSCCmetrics=SACSCCmetrics(~cellfun('isempty',SACSCCmetrics));
...
if PLOT_15panel
    ...
    Library.saveFigureAs([resultsDir 'eps' filesep 'sac' resultPostfix '.eps']);
    Library.saveFigureAs([resultsDir 'png' filesep 'sac' resultPostfix '.png']);
    Library.plot_CCCanal_NRSA(SACSCCfunctions,SACSCCmetrics,paramsOUT);
    Library.saveFigureAs([resultsDir 'eps' filesep 'sacScc' resultPostfix '.eps']);
    Library.saveFigureAs([resultsDir 'png' filesep 'sacScc' resultPostfix '.png']);
end
