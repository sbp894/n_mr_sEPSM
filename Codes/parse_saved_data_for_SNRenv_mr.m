...
%%
% clear; close all; clc;
% resultsDir='D:\Study Stuff\Matlab\mr-sEPSM_SNRenv-SimData\OUTPUT\Simulation\20170613-1\';
% resultTxt='CF_%1.4fk_Sent_%i_%s_level_%1.2f_SNR%i_fType%1.0f';

function parse_saved_data_for_SNRenv_mr(resultsDir,resultTxt)
...
figOutName=sprintf('%s__CompareWithNF_%i__fixSPL_%i__CorrWindow_0add1mul_%i__sent_num_%i', ...
    strrep(resultsDir(length(fileparts(resultsDir(1:end-1)))+2:end-1), '-','_'), CompareWithNF, ExpControlParams.fixSPL, ExpControlParams.winCorr0Add1Mul, sents);
                ...
                for sentVar=1:length(sents)
                    for cfVar=1:length(cfs)
                        for ftypeVar=1:length(fibertypes)
                            ...
                            PowerModCell=PSD_STRUCT.PowerModCell;
%                             paramsIN=load([resultsDir 'paramsIN' filesep 'paramsIN' resultPostfix '.mat']);
...
for ntypeVar=1:length(noisetypes)
    ...
    for snrVar=1:length(snrs)
        ...
        set(gca, 'Ytick', 1:size(OutputWeighted(ntypeVar, snrVar).SNRenv,1), 'yticklabel',  strread(num2str(uint16(cfs')),'%s'),'Xtick', 1:length(ModFreqs), 'xticklabel',  strread(num2str(ModFreqs),'%s')) %#ok<DSTRRD>
...
%%
markers='dos<>pv';
figure(length(noisetypes)+1); clf;
set(gcf,'units', 'normalized','position', [.1 .1 .8 .8]);
lw=2;
