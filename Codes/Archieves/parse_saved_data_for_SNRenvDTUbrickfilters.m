...
% function parse_saved_data_for_SNRenvDTUbrickfilters(resultsDir,resultTxt)
            ...
            for snrVar=1:length(snrs)
                for sentVar=1:length(sents)
                    for cfVar=1:length(cfs)
                        ...
                        pModSNbrick=cell(length(fibertypes),1);
                        pModSN_NFbrick=cell(length(fibertypes),1);
                        pModNbrick=cell(length(fibertypes),1);
                        pModN_NFbrick=cell(length(fibertypes),1);
                        SNRenvbrick=cell(length(fibertypes),1);
                        SNRenvNFbrick=cell(length(fibertypes),1);
                        ...
                        for ftypeVar=1:length(fibertypes)
                            ...
                            pModSNbrick{ftypeVar}=PSD_STRUCT.PowerMod_SN;
                            pModSN_NFbrick{ftypeVar}=PSD_STRUCT.PowerMod_SN_noisefloor;
                            pModNbrick{ftypeVar}=PSD_STRUCT.PowerMod_N;
                            pModN_NFbrick{ftypeVar}=PSD_STRUCT.PowerMod_N_noisefloor;
                            
                            SNRenvbrick{ftypeVar}=(pModSNbrick{ftypeVar}-pModNbrick{ftypeVar})./pModNbrick{ftypeVar};
                            SNRenvbrick{ftypeVar}(SNRenvbrick{ftypeVar}<0)=0;
                            SNRenvNFbrick{ftypeVar}=(pModSN_NFbrick{ftypeVar}-pModN_NFbrick{ftypeVar})./pModN_NFbrick{ftypeVar};
                            SNRenvNFbrick{ftypeVar}(SNRenvNFbrick{ftypeVar}<0)=0;
                        end
                        ...
                        SimData(ntypeVar, levelVar, snrVar, sentVar, cfVar).SNRenvbrick= SNRenvbrick;
                        SimData(ntypeVar, levelVar, snrVar, sentVar, cfVar).SNRenvNFbrick= SNRenvNFbrick;
...
for ntypeVar=1:length(noisetypes)
    ...
    for snrVar=1:length(snrs)
        ...
        imagesc(OutputWeighted(ntypeVar, snrVar).SNRenvnorm)
        set(gca, 'Ytick', 1:size(OutputWeighted(ntypeVar, snrVar).SNRenvnorm,1), 'yticklabel',  strread(num2str(uint16(cfs')),'%s'),'Xtick', 1:length(ModFreqs), 'xticklabel',  strread(num2str(ModFreqs),'%s'))
...
figure;
lw=2;
legstr=cell(2*length(noisetypes),1);
hold on;
for ntypeVar=1:length(noisetypes)
    ...
    legstr{2*ntypeVar-1}=noisetypes{ntypeVar};
