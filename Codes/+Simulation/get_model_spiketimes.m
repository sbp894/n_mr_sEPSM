function [SpikeTrains,paramsIN, mrWindows]=get_model_spiketimes(stim_S, stim_N, stim_SN, A, resultPostfix, C, AN, anal, resultsDir,  mrWindows) 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SET PARAMETERS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FixSNlevel=anal.FixSNlevel;
...
% SpikeTrainsA_plus=ensure_no_empty_spikes(meanrate_unad,dur_sec,ANmodel_fs_Hz,Nreps, anal); %=Library.get_sptimes(meanrate_unad,ANmodel_fs_Hz,Nreps);
SpikeTrainsA_plus=Library.get_sptimes(meanrate_unad_Ap,ANmodel_fs_Hz,Nreps);
...
% SpikeTrainsA_minus=ensure_no_empty_spikes(meanrate_unad,dur_sec,ANmodel_fs_Hz,Nreps, anal); %Library.get_sptimes(meanrate_unad,ANmodel_fs_Hz,Nreps);
SpikeTrainsA_minus=Library.get_sptimes(meanrate_unad_An,ANmodel_fs_Hz,Nreps);
...
% SpikeTrainsB_plus=ensure_no_empty_spikes(meanrate_unad,dur_sec,ANmodel_fs_Hz,Nreps, anal); %Library.get_sptimes(meanrate_unad,ANmodel_fs_Hz,Nreps);
SpikeTrainsB_plus=Library.get_sptimes(meanrate_unad_Bp,ANmodel_fs_Hz,Nreps);

% stim_B_minus
vIHC = ANModel.model_IHC(-stim_B_model.',CF_B_kHz*1000,1,1/ANmodel_fs_Hz,dur_sec+0.05,Cohc_B,Cihc_B,AN.species); % 2013 model
[sout_Bn,meanrate_unad_Bn, ~] = ANModel.model_Synapse(vIHC,CF_B_kHz*1000,1,1/ANmodel_fs_Hz,SRtype_B,noiseType ,implnt);
% SpikeTrainsB_minus=ensure_no_empty_spikes(meanrate_unad,dur_sec,ANmodel_fs_Hz,Nreps, anal); %Library.get_sptimes(meanrate_unad,ANmodel_fs_Hz,Nreps);
SpikeTrainsB_minus=Library.get_sptimes(meanrate_unad_Bn,ANmodel_fs_Hz,Nreps);
...
% SpikeTrainsC_plus=ensure_no_empty_spikes(meanrate_unad,dur_sec,ANmodel_fs_Hz,Nreps, anal); %Library.get_sptimes(meanrate_unad,ANmodel_fs_Hz,Nreps);
SpikeTrainsC_plus=Library.get_sptimes(meanrate_unad_Cp,ANmodel_fs_Hz,Nreps);

% stim_C_minus
vIHC = ANModel.model_IHC(-stim_C_model.',CF_C_kHz*1000,1,1/ANmodel_fs_Hz,dur_sec+0.05,Cohc_C,Cihc_C,AN.species); % 2013 model
[sout_Cn,meanrate_unad_Cn, ~] = ANModel.model_Synapse(vIHC,CF_C_kHz*1000,1,1/ANmodel_fs_Hz,SRtype_C,noiseType ,implnt);
% SpikeTrainsC_minus=ensure_no_empty_spikes(meanrate_unad,dur_sec,ANmodel_fs_Hz,Nreps, anal); %Library.get_sptimes(meanrate_unad,ANmodel_fs_Hz,Nreps);
SpikeTrainsC_minus=Library.get_sptimes(meanrate_unad_Cn,ANmodel_fs_Hz,Nreps);


if anal.figHandles.meanRates.doPlot
    figure(anal.figHandles.meanRates.han);
    clf;
    ymaxTEMP=-Inf;
...
clear paramsIN
paramsIN.dur_msec=dur_sec*1000; % remove the next three lines
paramsIN.durA_msec=dur_sec*1000;
paramsIN.durB_msec=dur_sec*1000;
paramsIN.durC_msec=dur_sec*1000;

paramsIN.CF_Hz=CF_A_kHz*1000; % remove the next three lines
